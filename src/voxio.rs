...
pub fn vox2gmsh(voxels: &Array3<i32>, dx: f64, file: &str) {
   ...
   for (voxel, v) in voxels.indexed_iter() {
       ...
       for (_, (neighbour_offset, faces)) in stitch_map.iter().enumerate() {
           ...
           let neighbour_offset = neighbour_offset.iter().map(|x| *x as usize).collect::<Vec<usize>>();
           let neighbour = (voxel.0 + neighbour_offset[0], voxel.1 + neighbour_offset[1], voxel.2 + neighbour_offset[2]);
...
pub fn vox2txt(voxels: &Array3<i32>, file: &str) {
   let mut f = File::create(file).unwrap();
